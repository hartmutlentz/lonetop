...
class ProgressMeter(object):
    def __init__(self, *args, **kw):
        ...
        self.meter_ticks = int(kw.get('ticks', 16))
    ...
    def update(self, count=1, *args, **kw):
    ...
    def _get_meter(self, *args, **kw):
        ...
        if self.count >= self.total:
            ...
            elif dur >=3600 and dur < 86400:
                if hours == 1:
                    est = ' completed in %.f hour %.f min                    ' % (hours, minutes)
                else:
                    est = ' completed in %.f hours %.f min                    ' % (hours, minutes)
            else:
                    est = ' completed in %.f days %.f hours                    ' % (hours/24, hours%24)
            ...
            if len(self.estimated_duration) >= 1:
                ...
                elif est_dur >= 3600 and est_dur < 86400:
                    if hours == 1:
                        est = ' %.f hour %02.f min remaining    ' % (hours, minutes)
                    else:
                        est = ' %.f hours %02.f min remaining   ' % (hours, minutes)
                else:
                    est = ' %.f days %02.f hours remaining   ' % (hours/24, hours%24)
                return '>[%s|%s] %d%%  %.3f %s/sec.%s' % (bar, pad, perc,
                                                      self.rate_current,
                                                      self.unit, est)
            else:
                return '>[%s|%s] %d%%  %.3f %s/sec. Calculating time' % (bar, pad, perc,
                                                                                      self.rate_current,
                                                                                      self.unit)
    ...
    def _refresh(self, *args, **kw):
        if self.switch_off:
            ...
            sys.stdout.write('\x08' * self.prev_meter_len + '\x08' * self.prev_meter_len)
            # Get meter text
