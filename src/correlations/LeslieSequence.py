...
class LeslieSequence(list):
    ...
    """ Like AdjMatrixSequence, but with focus on products and spectra.
        ...
        Input: AdjMatrixSequence or networkx Graph Generator
        with parameters in args.
    ...
    def leslie_matrix(self, M):
        ...
        return sp.kron(flux, M) + sp.kron(aging,
                                          sp.identity(self.number_of_nodes))
    ...
    def initial_state(self, nodes=None):
        ...
        if nodes:
            ...
            for i in nodes:
                assert i < self.number_of_nodes, 'Initial node\
                    not in node set.'
                x[i, 0] = 1
        else:
            x = sp.lil_matrix((self.max_dimension, 1), dtype='int')
    ...
    def oldest_in_x(self, s):
        """ the number of non-zeros for the last self.number_of_nodes
            elements of s
        """
        indices = range(
            self.max_dimension -
    ...
    def outbreak(self, initial_nodes=None):
        ...
        for i in range(len(self)):
            ...
            if recovered == self.number_of_nodes:
                break
            if state.nnz == 0:
                break
...
if __name__ == "__main__":
    ...
    # At.as_undirected()
    # At =
    # AdjMatrixSequence(fs.dataPath("D_sw_uvd_01JAN2009_31MAR2010.txt"),
    #    matr_type='dok')
